...
class TableFilter extends React.Component {
  ...
  handleDropdownChange = (newValue, index, column) => {
    const labelFilterAll = this.props.options.textLabels.filter.all;
    const value = newValue === labelFilterAll || !newValue ? [] : [newValue];
      ...
      <GridListTile key={index} cols={1} classes={{ tile: classes.gridListTile }}>
        <FormControl key={index} fullWidth>
          <Autocomplete
            ...
            size={'small'}
            onChange={(event, value) => this.handleDropdownChange(value, index, column.name)}
            options={filterData[index]}
            renderInput={params => <TextField {...params} label={column.name} fullWidth />}
          />
        </FormControl>
      </GridListTile>
    );
  ...
  renderCustomField(column, index) {
    const { classes, filterList, options, filterData } = this.props;
    const display =
