...
class MultiSelectWithSearch extends React.Component {
  ...
  render() {
    const { classes, isDisabled, name, options, className, tooltipProps, icon } = this.props;
      ...
      <React.Fragment>
        <ClickAwayListener onClickAway={this.handleHideSuggestion}>
          <span className={classes.container}>
            <Tooltip {...otherTooltipsProps} placement={showSuggestion ? 'top' : placement}>
              <IconButton onClick={this.toggleSuggestion}>{icon}</IconButton>
            </Tooltip>
            {showSuggestion && (
                      ...
                      {suggestions.length !== 0 ? (
                        <React.Fragment>
                          {suggestions.map(s => {
                            ...
                            return (
                              <ListItem key={s.key} button onClick={event => this.handleClick(s.key)}>
                                <ListItemIcon
                                  classes={{
                                    root: classes.listItemIcon,
                                  }}>
                                  <Checkbox color="primary" edge="start" disableRipple checked={checked} />
                                </ListItemIcon>
                                <ListItemText
                                  primary={
